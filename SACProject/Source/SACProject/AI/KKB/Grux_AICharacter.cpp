...
bool AGrux_AICharacter::IsStampedeActive() const
{
	...
	if (IsValid(m_StampedeCooldown))
		IsCooldown = !m_StampedeCooldown->IsCooldown();
...
bool AGrux_AICharacter::IsEarthquakeActive() const
{
	...
	if (IsValid(m_EarthquakeCooldown))
		IsCooldown = !m_EarthquakeCooldown->IsCooldown();
...
{
	// 충돌체를 생성해 데미지를 준다.
	AGrux_BaseAttackCol* col = GetWorld()->SpawnActor<AGrux_BaseAttackCol>(GetActorLocation() + GetActorForwardVector() * 300.f, FRotator(0.0, GetActorRotation().Yaw, 0.0));
}
...
{
	// 충돌체를 생성해 데미지를 준다.
	AGrux_DoublePainCol	* col = GetWorld()->SpawnActor<AGrux_DoublePainCol>(GetActorLocation() + GetActorForwardVector() * 300.f, FRotator(0.0, GetActorRotation().Yaw, 0.0));
}
...
void AGrux_AICharacter::Skill2_Stampede_Attack_ActiveSkill()
{
	...
	AGrux_StampedeAttackCol* col = GetWorld()->SpawnActor<AGrux_StampedeAttackCol>(GetActorLocation() + GetActorForwardVector() * 300.f, FRotator(0.0, GetActorRotation().Yaw, 0.0));
}
